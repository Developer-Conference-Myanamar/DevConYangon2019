...
class InfoViewPagerAdapter : RecyclerView.Adapter<OnBasedViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): OnBasedViewHolder {
        return when(viewType){
            settingView->{
                SettingViewHolder(LayoutInflater.from(parent.context).inflate(R.layout.fragment_setting, parent, false))
            }
            aboutView->{
                AboutViewHolder(LayoutInflater.from(parent.context).inflate(R.layout.fragment_about, parent, false))
            }
            sponsorView->{
                SponsorViewHolder(LayoutInflater.from(parent.context).inflate(R.layout.fragment_demo, parent, false))
            }
            else-> SponsorViewHolder(LayoutInflater.from(parent.context).inflate(R.layout.fragment_demo, parent, false))
